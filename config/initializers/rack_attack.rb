...
class Rack::Attack
  ...
  throttle('throttle_per_token_api', limit: 300, period: 1.minute) do |req|
  ...
  throttle('throttle_api_media', limit: 300, period: 1.minute) do |req|
    req.authenticated_user_id if req.post? && req.path.match?(%r{\A/api/v\d+/media\z}i)
  end

  throttle('throttle_media_proxy', limit: 3000, period: 1.minute) do |req|
