...
RSpec.describe REST::QuoteSerializer do
  ...
  context 'with a pending quote' do
    ...
    context 'when the quoted post has an implicit public quote policy' do
      let(:quote) { Fabricate(:quote, status: Fabricate(:status, account: Fabricate(:account)), quoted_status: Fabricate(:status, account: Fabricate(:account, domain: 'quoted.example'), visibility: :public, quote_approval_policy: InteractionPolicy::POLICY_FLAGS[:public] << 16)) }
