  ...
  context 'with an oauth token' do
    ...
    describe 'POST /api/v1/statuses' do
      ...
      context 'with a quote of a remote public post with a recorded implicit quote policy' do
        let!(:quoted_status) { Fabricate(:status, account: Fabricate(:account, domain: 'misskey.example'), visibility: :public, quote_approval_policy: InteractionPolicy::POLICY_FLAGS[:public] << 16) }
        let(:params) do
          {
            status: 'Hello, this is a quote',
            quoted_status_id: quoted_status.id,
      ...
      context 'with a quote of a remote unlisted post with a recorded implicit quote policy' do
        let!(:quoted_status) { Fabricate(:status, account: Fabricate(:account, domain: 'misskey.example'), visibility: :unlisted, quote_approval_policy: InteractionPolicy::POLICY_FLAGS[:public] << 16) }
        let(:params) do
          {
            status: 'Hello, this is a quote',
            quoted_status_id: quoted_status.id,
