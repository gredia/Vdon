...
RSpec.describe ActivityPub::ProcessAccountService do
  ...
  describe 'indexability compatibility' do
    let(:payload) do
      {
    ...
    it 'defaults to indexable when the remote actor omits the property' do
      account = subject.call(payload)

      expect(account).to be_indexable
    end

    context 'when the remote actor explicitly opts out' do
      before do
        payload[:indexable] = false
      end

      it 'preserves the opt-out' do
        account = subject.call(payload)

        expect(account).to_not be_indexable
      end
    end
  end
